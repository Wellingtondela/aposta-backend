...
// Inicializa o Firebase (configurações em firebaseConfig.js)
initializeFirebase();
...
app.post('/apostar', (req, res) => {
  ...
  // Aqui você vai salvar no Firebase - depois eu te ajudo a implementar

  res.json({ sucesso: true, mensagem: 'Aposta recebida com sucesso!' });
});
...
app.post('/apostar', async (req, res) => {
  ...
  try {
    ...
    await db.collection('apostas').add({ aposta, telefone, timestamp: admin.firestore.FieldValue.serverTimestamp() });
